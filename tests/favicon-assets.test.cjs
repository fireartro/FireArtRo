...
test('the public document exposes FireArtRo icons for browsers and installed shortcuts', () => {
  ...
  assert.match(html, /rel="icon"[^>]+type="image\/png"[^>]+href="%PUBLIC_URL%\/favicon-96x96\.png"[^>]+sizes="96x96"/);
  ...
  for (const asset of ['favicon.ico', 'favicon-16x16.png', 'favicon-32x32.png', 'favicon-96x96.png', 'apple-touch-icon.png', 'icon-192.png', 'icon-512.png']) {
  ...
  assert.deepEqual(pngSize('favicon-96x96.png'), [96, 96]);
