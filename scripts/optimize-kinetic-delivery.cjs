...
const output = path.join(repo, 'output/fireart-cinema/delivery-2026-09-09');
...
for (const [profile, [width, height]] of Object.entries(profiles)) {
  const master = path.join(repo, 'output/fireart-cinema/kinetic-v3', profile, 'master.mp4');
  const filename = `fireart-hero-${profile}-av1.mp4`;
  const destination = path.join(output, filename);
  const compatible = path.join(repo, 'frontend/public/media', `fireart-hero-${profile}.mp4`);
  const phone = profile.startsWith('mobile');
  const crf = ['wide', 'ultrawide'].includes(profile) ? 32 : phone ? 29 : 30;
  ...
  const quality = spawnSync('ffmpeg', ['-hide_banner','-i',destination,'-i',master,'-lavfi',
  ...
  const vmaf = Number(quality.stderr.match(/VMAF score: ([\d.]+)/)?.[1]);
  if (quality.status !== 0 || !Number.isFinite(vmaf) || vmaf < 95) throw new Error(`Quality gate failed: ${profile} (${vmaf})`);
  const row = { profile, width, height, seconds:32, frames:768, codec:'av1', crf, preset, bytes:bytes.length, fallbackBytes,
    reductionPercent:Number((100*(1-bytes.length/fallbackBytes)).toFixed(1)), vmaf,
