...
for (const [profile, [width,height]] of Object.entries(profiles)) {
  ...
  // High-motion fireworks require more detail than the previous mostly-still
  // montage. The compatibility file is capped separately from efficient AV1.
  const byteBudget = profile === 'mobile-tall' ? 25 : profile.startsWith('mobile') ? 24 : 48;
  if (data.indexOf(Buffer.from('moov')) > data.indexOf(Buffer.from('mdat')) || data.length > byteBudget*1048576) throw new Error(`Non-streamable or oversized video: ${profile}`);
